...
// --------------------------------------------------------------------------
// ORDER HISTORY MANAGER — persists placed orders across sessions
// --------------------------------------------------------------------------
...
function hydrateCheckoutPage() {
  ...
  document.getElementById("btnPlaceOrder").addEventListener("click", () => {
    ...
    const paymentMethod = selectedPayCard
      ? selectedPayCard.querySelector(".pay-method-label")?.textContent?.trim() || "Card"
      : "Unknown";
    ...
    if (successModal) {
      // Update the tracking code in the modal with the generated one
      const trackEl = successModal.querySelector("strong[style*='accent-cream']");
